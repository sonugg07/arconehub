...
import { X, CheckCheck, Trash2, Bell, CheckCircle2, Info, AlertTriangle, AlertCircle } from "lucide-react";
        ...
        <div className="p-6 border-b border-white/[0.08] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">ArcOne Hub live activity stream</p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
          {notifications.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center p-6 text-slate-400">
              <Bell className="w-10 h-10 stroke-1 text-slate-600 mb-3" />
              <div className="text-sm font-bold text-white">No notifications</div>
              <div className="text-xs text-slate-400 mt-1">Your onchain alerts will appear here</div>
            </div>
          ) : (
        ...
        {notifications.length > 0 && (
          <div className="p-4 border-t border-white/[0.08] flex items-center justify-between gap-3 text-xs bg-[#050713]">
            <button
              ...
              className="px-3 py-2 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] text-slate-300 flex items-center gap-1.5 transition-colors font-medium"
            >
            ...
            <button
              onClick={clearAll}
              className="px-3 py-2 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 flex items-center gap-1.5 transition-colors font-medium"
            >
